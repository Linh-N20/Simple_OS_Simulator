...
#ifdef MM_PAGING
	/* Each process owns its personal mm_struct so that multiple
	 * concurrent processes do not overwrite each other's page tables
	 * through the shared krnl->mm pointer.                          */
	struct mm_struct *own_mm;
#endif
};
