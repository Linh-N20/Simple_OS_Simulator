...
int MEMPHY_get_freefp(struct memphy_struct *mp, addr_t *retfpn)
{
   ...
   *retfpn = fp->fpn;
   ...
   /* Move the node to the used list instead of freeing it! */
...
/*
 *  MEMPHY_dump - dump non-zero bytes in MEMPHY storage for tracing
 *  @mp: memphy struct
 *
 *  Prints the physical address and value of every non-zero byte.
 *  Useful for verifying the result of alloc/read/write sequences.
 */
int MEMPHY_dump(struct memphy_struct *mp)
{
   ...
   for (i = 0; i < mp->maxsz; i++) {
      if (mp->storage[i] != 0) {
         printf("  [0x%04x] = 0x%02x (%3d)\n",
                i, (unsigned char)mp->storage[i],
                (unsigned char)mp->storage[i]);
         printed++;
      }
   }
   ...
   printf("=== END DUMP (%d non-zero bytes) ===\n", printed);
   return 0;
}
