...
static void * cpu_routine(void * args) {
	...
	while (1) {
		...
		/* Run current process */
#ifdef MM_PAGING
		/* Restore this process's personal mm before running.
		 * Without this, concurrent processes overwrite each
		 * other's krnl->mm causing crashes on multi-process
		 * workloads like os_0_mlq_paging.                   */
		if (proc->own_mm != NULL)
			proc->krnl->mm = proc->own_mm;
#endif
		run(proc);
		time_left--;
		next_slot(timer_id);
...
static void * ld_routine(void * args) {
  ...
  /* TODO init kernel page table directory */
	...
	while (i < num_processes) {
		...
		/* Save this process's personal mm so it survives
		 * context switches to other processes             */
		proc->own_mm = krnl->mm;
		krnl->mram = mram;
		krnl->mswp = mswp;
...
static void read_config(const char * path) {
	...
	int count = sscanf(line, "%lu %lu %lu %lu %lu", &a, &b, &c, &d, &e);
...
int main(int argc, char * argv[]) {
	...
	mm_ld_args->active_mswp = mswp_ptr[0];
